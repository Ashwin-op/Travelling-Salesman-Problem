...
def load(filepaths):
    files = {}
    with click.progressbar(filepaths, label='Loading files', show_pos=True, show_eta=True) as bar:
        for filepath in bar:
            try:
...
@cli.command()
@click.argument('filepaths', nargs=-1, type=click.Path(dir_okay=False, readable=True))
@click.option('-c', '--column', 'columns', multiple=True, default=DEFAULT_COLUMNS)
...
def summarize(filepaths, columns, by_keyword):
    ...
    header, rows = get_tabular_data(load(filepaths), columns=columns, by_keyword=by_keyword)
